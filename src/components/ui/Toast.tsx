...
const ToastViewport = ({ className, ref, ...props }: React.ComponentProps<typeof ToastPrimitives.Viewport>) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      'fixed top-0 z-100 flex max-h-screen w-full flex-col-reverse p-4 md:bottom-0 md:right-0 md:top-auto md:max-w-105 md:flex-col',
      className
    )}
    {...props}
  />
...
const ToastClose = ({ className, ref, ...props }: React.ComponentProps<typeof ToastPrimitives.Close>) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      'absolute right-2 top-2 rounded-md p-1 text-foreground/50 opacity-0 transition-opacity hover:text-foreground focus:opacity-100 focus:outline-hidden focus:ring-2 group-hover:opacity-100 group-[.destructive]:text-destructive-foreground hover:group-[.destructive]:text-destructive/10 focus:group-[.destructive]:ring-primary-foreground focus:group-[.destructive]:ring-offset-primary-foreground',
      className
    )}
    toast-close=''
    {...props}
